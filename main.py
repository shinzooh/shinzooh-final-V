...
async def webhook(request: Request, data: TradingViewData):
    try:
        ...
        analysis = response.json()["choices"][0]["message"]["content"][:1900].strip()  # Limit to 1900 chars and remove extra spaces
        if len(response.json()["choices"][0]["message"]["content"]) > 1900:
            logger.warning("Analysis truncated due to Telegram limit")
        if not analysis:
            logger.warning("Analysis is empty, using default message")
